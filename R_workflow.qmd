...
se.bacvir_0.01.weighted <- graph_from_adjacency_matrix(adjacency_weight_bac_0.01,
                                              mode="undirected",
                                              weighted=TRUE,
                                              diag=FALSE)
se.bacvir_0.01.weighted_weight <- E(se.bac_0.01.weighted)$weight
...
## bacteria
library(colorspace)
V(se.bac_0.01.weighted)$frame.color <- rgb(0.5, 0.5, 0.5, 0.5)  
...
nc <- function(adj_matrix) {
  ...
  lambda_sum <- sum(exp(lambda))
  N <- length(lambda)
  nc <- log(lambda_sum / N, base = exp(1))
  complete_graph <- matrix(1, nrow = N, ncol = N) 
  diag(complete_graph) <- 0                       
  lambda_complete <- eigen(complete_graph, only.values = TRUE)$values
  lambda_sum_complete <- sum(exp(lambda_complete))
  nc_max <- log(lambda_sum_complete / N, base = exp(1))
  nc_normalized <- nc / nc_max
...
#Degree
node_order_degree_gl <- order(degree(ig.bac_0.01), decreasing = TRUE)
node_order_degree_bacvir <- order(degree(ig.bacvir_0.01), decreasing = TRUE)
deleted_nodes_gl <- data.frame(node = character(), natural_connectivity = numeric())
deleted_nodes_bacvir <- data.frame(node = character(), natural_connectivity = numeric())
natural_d_connectivity_vals_gl <- c(nc(as_adjacency_matrix(ig.bac_0.01, sparse = FALSE)))
natural_d_connectivity_vals_bacvir <- c(nc(as_adjacency_matrix(ig.bacvir_0.01, sparse = FALSE)))
# attack nodes based on degree
deleted_nodes_gl <- data.frame(node = "0", natural_connectivity = nc(as_adjacency_matrix(ig.bac_0.01, sparse = FALSE)))
...
adj_matrix_random <- as_adjacency_matrix(ig.random_0.1, sparse = FALSE)
...
dat <- data.frame(
  removed_proportion = c(removed_proportion_gl, removed_proportion_bacvir),
  natural_connectivity = c(natural_connectivity_gl, natural_connectivity_bacvir),
  network = rep(c("bacteria network", "bacteria-virus network"), c(length(natural_connectivity_gl), length(natural_connectivity_bacvir)))
)
rm(areas)
areas <- dat %>%
  group_by(network) %>%
  summarize(
    area = auc(removed_proportion, natural_connectivity) 
  )
print(areas)
areas <- areas %>% 
  mutate(label = paste0(network, " (AUC = ", round(area, 3), ")"))
ordered_areas <- areas[match(unique(dat$network), areas$network), ]

auc_text <- paste(
  "AUC:\n",
  paste0(ordered_areas$network, ": ", round(ordered_areas$area, 3), collapse = "\n")
)
...
r <- ggplot(dat, aes(removed_proportion, natural_connectivity, color = network)) +
  ...
  theme_minimal()+
  annotate(
    "text", x = 1.05, y = max(dat$natural_connectivity), 
    label = auc_text, hjust = 0, vjust = 1, size = 3, 
